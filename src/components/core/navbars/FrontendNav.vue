                  ...
                  <b-collapse id="navbarSupportedContent" is-nav :visible="sidebar">
                     <div class="menu-main-menu-container">
                        <ul id="top-menu" class="navbar-nav ml-auto">
                           <li v-for="(item,index) in items" :key="index" class="menu-item" :style="{'order': `${index > 1 ? index+1 : index}`}">
                              <router-link :to="item.link">{{item.title}}</router-link>
                           </li>
                           <li class="menu-item nav-item nav-icon" v-nav-toggle style="order: 2">
                              <a href="#" class="iq-user-dropdown search-toggle p-0 d-flex align-items-center" @mouseenter="showGamesModal = true" @mouseleave="showGamesModal = false">
                                 Games
                              </a>
                              <transition name="fade">
                                 <div class="iq-sub-dropdown iq-user-dropdown games-dropdown" v-show="showGamesModal" @mouseenter="showGamesModal = true" @mouseleave="showGamesModal = false">
                                    <span >Bruce</span>
                                    <span>Bruce Saves Christmas</span>
